<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="refs">
    ...
    <refs>
      <xsl:for-each select="./*[name()='ref'] | ./optional/*[name()='ref'] | ./zeroOrMore/*[name()='ref']">
        <xsl:variable name="ref_name" select="./@name"/>
        <xsl:if test="count(/grammar/define[@name=$ref_name]/*[1]/*) > 1 or ./@name = 'def_ipi-list' or ./@name = 'def_artist-credit' or ./@name = 'def_attribute-list'">
          ...
          <xsl:element name="{$replaced_ref_name}">
            <xsl:choose>
              ...
              <xsl:when test="./@name = 'def_artist-credit'">
                <resource>
                  <parent>name-credit</parent>
                  <parent_attribute>joinphrase</parent_attribute>
                  <name>artist</name>
                </resource>
              </xsl:when>
